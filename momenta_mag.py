...
# Function to retrieve MC and collision data from ROOT files
def retrieve_data(filetype, n_samples):
    selection = "(V_M > 70) & (V_M < 110)" # Preselection on invariant mass
    branches = ["mum_uncalibrated_P", "mup_uncalibrated_P", "V_M", "mum_eta", "mup_eta", "mum_q", "mup_q"]
    arrays = filetype['DecayTree'].arrays(branches, selection, library='np')
    
    mum_p, mup_p = arrays['mum_uncalibrated_P'], arrays['mup_uncalibrated_P']
    V_M    = arrays['V_M']
    mum_eta, mup_eta = arrays['mum_eta'], arrays['mup_eta']
    mum_q, mup_q   = arrays['mum_q'], arrays['mup_q']
    ...
    momenta = (torch.tensor(np.stack([mum_p, mup_p], axis=1), dtype=torch.float32))/1000 # Convert to GeV
...
n_dimensions = 2 # Dimensions of space on which the flow is learning (in this case the 2D momentum vector)
n_samples = 100000  # Number of samples to retrieve from ROOT files
...
momenta_data, angles_data, contexts_data, indices_data = retrieve_data(data_file, n_samples)
momenta_MC, angles_MC, contexts_MC, indices_MC = retrieve_data(MC_file, n_samples)
...
# Create the flow
flow = make_flow(num_layers=10, hidden_features=128)  # Create a flow
flow = Flow(transform=flow, distribution=StandardNormal([n_dimensions])).to(device)
flow = flow.to(device)
...
# Precompute target PDF for loss calculation (avoiding recalculating every epoch for speed and less noise)
with torch.no_grad():
    target_mass = calculate_invariant_mass(momenta_data, angles_data) 
...
# KDE Plots (uncomment if preferred over histograms)
